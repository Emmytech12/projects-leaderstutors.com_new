	...
	if (next_id == 'nextPage2') {
		if (fullname == '') {
			...
			if (userSignUpSession) {
				$('#reg_fullname').val(fullname);
				$('#reg_email').val(email);
				$('#reg_mobile_no').val(mobile_no);
			}
		...
		if (create_password == '') {
			...
			_userProceedToPayment(fullname, email, mobile_no, department_id, class_id, create_password, confirm_password);
		}
	} else {
		/// do nothing
	}
...
function _userProceedToPayment(fullname, email, mobile_no, department_id, class_id, create_password, confirm_password) {
	//////////////// get btn text ////////////////
	...
	////////////////////////////////////////////////	
	var action = 'signup';
	var dataString = 'fullname=' + fullname + '&email=' + email + '&mobile_no=' + mobile_no + '&department_id=' + department_id + '&class_id=' + class_id + '&create_password=' + create_password + '&confirm_password=' + confirm_password;
	...
	$.ajax({
		...
		success: function (data) {
			...
			if (success == true) {
				...
				_payWithPaystack(user_id, fullname, mobile_no, access_key, subscription_id, transaction_id, email, amount, paystack_payment_key)
...
function _payWithPaystack(user_id, fullname, mobile_no, access_key, subscription_id, transaction_id, email, amount, paystack_payment_key) {
	...
	var handler = PaystackPop.setup({
		...
		callback: function (response) { //success
			...
			_callPaymentSuccess(access_key, user_id, email, transaction_id, subscription_id, amount, paystack_payment_key);
		},
...
function _callPaymentSuccess(access_key, user_id, email, transaction_id, subscription_id, amount, paystack_payment_key) {
	var action = 'signup-complete';
	var dataString = 'user_id=' + user_id + '&email=' + email + '&transaction_id=' + transaction_id + '&subscription_id=' + subscription_id + '&amount=' + amount + '&paystack_payment_key=' + paystack_payment_key;
	$.ajax({
		type: "POST",
		url: endPoint + '/user/auth/' + action,
		data: dataString,
		...
		success: function (info) {
			...
			if (success == true) {
				if (userSignUpSession) {
					$('#reg_fullname,#reg_email,#reg_mobile_no').val('');
				}
